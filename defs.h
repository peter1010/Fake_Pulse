...
typedef pa_channel_map * (*FP_channel_map_init_auto)(pa_channel_map *, unsigned, pa_channel_map_def_t);
...
typedef pa_context_state_t (*FP_context_get_state)(const pa_context *);
typedef pa_context * (*FP_context_new)(pa_mainloop_api *, const char *);
typedef pa_time_event * (*FP_context_rttime_new)(const pa_context *,pa_usec_t, pa_time_event_cb_t, void *);
...
typedef pa_operation_state_t (*FP_operation_get_state)(const pa_operation *);
typedef void (*FP_operation_unref)(pa_operation *);
typedef const char * (*FP_proplist_gets)(const pa_proplist *, const char *);
typedef pa_usec_t (*FP_rtclock_now)(void);
typedef int (*FP_stream_begin_write)(pa_stream *, void **, size_t *);
...
typedef uint32_t (*FP_stream_get_index)(const pa_stream *);
...
typedef size_t (*FP_stream_readable_size)(const pa_stream *);
typedef size_t (*FP_stream_writable_size)(const pa_stream *);
typedef int (*FP_stream_drop)(pa_stream *);
typedef const pa_buffer_attr * (*FP_stream_get_buffer_attr)(pa_stream *);
typedef const char * (*FP_stream_get_device_name)(const pa_stream *);
